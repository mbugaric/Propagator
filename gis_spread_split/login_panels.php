...
{
		session_start();
		$_SESSION['user_name_panels']="";

		
		$db=new db_func();
		$db->connect();
		
		$query = "SELECT password, user_id FROM users WHERE username = '".$_POST["usrname"]."' and paneluser_id = 1";
		...
		while ($line = pg_fetch_array($result, null, PGSQL_ASSOC)) {
			foreach ($line as $col_value) {
				if(hash_hmac('ripemd160',$_POST["pass"],"AdriaProp") == $line[password])
				{
					$_SESSION['user_name_panels'] = $_POST['usrname'];
					$_SESSION['id_user'] = $line[user_id];
				}
			}
		}
		...
		{	
			 header("Location: ./panels/panels.php?user=");
			 exit(1);
		}
		...
		else
		{
				...
				print "<SCRIPT>alert('<?php echo _NATPIS_WRONG_LOGIN?>'); document.location.href='login_panels.php';</SCRIPT>\n";
		}
...
<FORM NAME="form" ACTION="login_panels.php" METHOD="POST" 
  ...
  <TR>
    <TD>
      ...
      <TABLE WIDTH="100%" BORDER="0" CELLPADDING="3" CELLSPACING="0">
        <TR>
            <TD align="center"><img src="./css/images/AdriaFireRisk.png" width="632" height="146"></TD>
          </TR>
      </TABLE>
...
<TR>
        ...
        <TR>
          <TD COLSPAN="2" VALIGN="middle" ALIGN="center"><INPUT TYPE=submit SIZE=7 title="<?php echo "AdriaFireRisk"?>" class="inputMenu" onMouseOver="this.className = 'inputMenuOver';" onMouseOut="this.className = 'inputMenu';" NAME=submit VALUE="<?php echo _NATPIS_POTVRDA?>"></TD>
        </TR>
        <TR>
          <TD COLSPAN="2" HEIGHT="10" BACKGROUND="gif/spacer.gif"></TD>
        </TR>
		...
		<TR> 
            <TD COLSPAN="2"><div  align="center" ><?php echo _NATPIS_DISCLAIMER_RISK?></div></TD>
        </TR>
        <TR>
